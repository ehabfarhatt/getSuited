...
function stddev(values: number[]): number {
  ...
  const moveToNextQuestion = async () => {
    ...
    const bodyLanguage = stopFaceTracking(); // assuming this returns latest metrics
  
    try {
      const deepgramResult = await analyzeAudioWithDeepgram(blob);
      const transcript = deepgramResult.results.channels[0].alternatives[0].transcript;
      const asrConfidence = deepgramResult.results.channels[0].alternatives[0].confidence;

      const assemblyResult = await analyzeToneWithAssembly(blob);
      //const detectedEmotion = assemblyResult.emotion;
      const sentiment = assemblyResult.sentiment;
      const assemblyConfidence = assemblyResult.confidence;

      const mergedBodyLanguage: BodyLanguageMetrics = {
        ...bodyLanguage,
        //emotion: detectedEmotion || bodyLanguage.emotion,
      };
      ...
      const newEntry: EvaluationResult = {
        ...
        toneConfidence: assemblyConfidence,
        bodyLanguage: {
          ...mergedBodyLanguage,
          confidence: bodyLanguage.confidence,
        },
      ...
      if (currentQuestionIndex < questions.length - 1) {
        setResults(updatedResults); // store result
        ...
        setResults(updatedResults); // ensure final is set
